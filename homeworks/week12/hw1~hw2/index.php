...
<html>
  <head>
    ...
    <title>Week12-Message Board with PHP（Cindy）</title>
  ...
  <body>
    <div class='messageboard'>
      ...
      <?php
        ...
        $sql = "SELECT users.username, users.nickname, comments.user_id, comments.id, comments.content, comments.nickname, comments.created_at
        ...
        if ($result->num_rows > 0) {
          while($row2 = $result->fetch_assoc()) {
            ...
            if ($status === 'login' && $row2['user_id'] == $_SESSION['user_id']) {
              ...
              echo     "<div><input class='messageboard__latestpost-delete self" . $row2['id'] . "'' type='submit' title='刪除留言' value=''><i class='far fa-trash-alt'></i></input></div>";
              echo   "</form>";
      ...
      <div class='messageboard__page'>
        <?php
          $pageSql = "SELECT * FROM CindyLyu_comments WHERE comment_id_at=0";
