function htmlspecialchars(ch) {
  if (ch === null) return '';
  let result;
  result = ch.replace(/&/g, '&amp;'); // g 代表 flag，所有符合的字都取代
  result = ch.replace(/"/g, '&quot;');
  result = ch.replace(/'/g, '&#039;');
  result = ch.replace(/</g, '&lt;');
  result = ch.replace(/>/g, '&gt;');
    ...
    editButton[i].onclick = function () { // 編輯按鈕點擊事件
      ...
      for (let k = 0; k < cancelEdit.length; k += 1) {
        cancelEdit[k].onclick = function () { // 取消編輯按鈕點擊事件
          ...
          editArea.outerHTML = `<div class='messageboard__latestpost-content self${cancelId}'>${htmlspecialchars(latestPostContent.innerText)}</div>`;
